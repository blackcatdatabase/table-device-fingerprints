...
use BlackCat\Database\Packages\DeviceFingerprints\Repository\DeviceFingerprintRepositoryInterface;
...
class DeviceFingerprintRepository implements DeviceFingerprintRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'fingerprint_hash' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\DeviceFingerprints\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'user_id', 'attributes', 'risk_score', 'last_seen', 'last_ip_hash', 'last_ip_key_version' ]);
          return count($rows);
        }
    ...
    public function getByFingerprintHash(string $fingerprintHash, bool $asDto = false): array|\BlackCat\Database\Packages\DeviceFingerprints\Dto\DeviceFingerprintDto|null {
        return $this->getByUnique([ 'fingerprint_hash' => $fingerprintHash ], $asDto);
    }
    ...
    public function getIdByFingerprintHash(string $fingerprintHash) {
        $row = $this->getByFingerprintHash($fingerprintHash, false);
