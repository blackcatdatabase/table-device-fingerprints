...
namespace BlackCat\Database\Packages\DeviceFingerprints\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\DeviceFingerprints\Definitions;
use BlackCat\Database\Packages\DeviceFingerprints\Criteria;
use BlackCat\Database\Packages\DeviceFingerprints\Dto\DeviceFingerprintDto as Dto;
use BlackCat\Database\Packages\DeviceFingerprints\Mapper\DeviceFingerprintDtoMapper as RowMapper;
...
final class DeviceFingerprintRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\DeviceFingerprints\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'fingerprint_hash' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\DeviceFingerprints\Definitions::table());
            return $bulk->upsertMany($rows, [ 'fingerprint_hash' ], [ 'user_id', 'attributes', 'risk_score', 'last_seen', 'last_ip_hash', 'last_ip_key_version' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\DeviceFingerprints\Dto\DeviceFingerprintDto|null */
    public function getByFingerprintHash(string $fingerprintHash, bool $asDto = false): array|\BlackCat\Database\Packages\DeviceFingerprints\Dto\DeviceFingerprintDto|null {
        $row = $this->getByUnique([ 'fingerprint_hash' => $fingerprintHash ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\DeviceFingerprints\Mapper\DeviceFingerprintDtoMapper::fromRow($row);
    }
    public function existsByFingerprintHash(string $fingerprintHash): bool {
        $where = 't.' . Ident::q($this->db, 'fingerprint_hash') . ' = :uniq_fingerprint_hash';
        return $this->exists($where, [ 'uniq_fingerprint_hash' => $fingerprintHash ]);
    }
    /** @return int|string|null */
    public function getIdByFingerprintHash(string $fingerprintHash) {
        $row = $this->getByFingerprintHash($fingerprintHash);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\DeviceFingerprints\Dto\DeviceFingerprintDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\DeviceFingerprints\Dto\DeviceFingerprintDto|null {
