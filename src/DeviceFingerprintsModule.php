...
final class DeviceFingerprintsModule implements ModuleInterface
{
    public function name(): string { return 'table-device_fingerprints'; }
    public function table(): string { return 'device_fingerprints'; }
...
SQL;
    ...
    public function status(Database $db, SqlDialect $d): array
    {
        ...
        $expectedIdx = [ 'idx_df_last_seen', 'idx_df_user', 'idx_df_user_last_seen' ];
        if ($d->isMysql()) {
    ...
    public function info(): array
    {
        return [
            ...
            'indexes'     => [ 'idx_df_last_seen', 'idx_df_user', 'idx_df_user_last_seen' ],
            'foreignKeys' => [ 'fk_df_user' ],
        ];
    }
}
